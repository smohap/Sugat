...
create function sugat.is_org_member(org uuid)
...
create function sugat.current_member_role(org uuid)
returns member_role language sql security definer stable set search_path = sugat, public, extensions as $$
  select m.role from memberships m
  where m.org_id = org and m.profile_id = auth.uid() and m.status = 'active';
$$;

create function sugat.is_org_admin(org uuid)
returns boolean language sql security definer stable set search_path = sugat, public, extensions as $$
  select sugat.current_member_role(org) = 'admin';
$$;

-- Moderators and admins share the moderation surface.
create function sugat.is_org_moderator(org uuid)
returns boolean language sql security definer stable set search_path = sugat, public, extensions as $$
  select sugat.current_member_role(org) in ('admin', 'moderator');
$$;

-- Committee members create events; admins may touch any of them.
create function sugat.can_manage_events(org uuid)
returns boolean language sql security definer stable set search_path = sugat, public, extensions as $$
  select sugat.current_member_role(org) in ('admin', 'committee');
$$;

-- Rule 4: scoped to one event, grants nothing else anywhere.
create function sugat.is_event_checker(evt uuid)
returns boolean language sql security definer stable set search_path = sugat, public, extensions as $$
  select exists (
...
create function sugat.shares_org(p uuid)
returns boolean language sql security definer stable set search_path = sugat, public, extensions as $$
  select exists (
    select 1
    from memberships a
...
create function sugat.is_thread_member(t uuid)
returns boolean language sql security definer stable set search_path = sugat, public, extensions as $$
  select exists (
...
create policy profiles_self_read on profiles
  for select using (id = auth.uid() or sugat.shares_org(id));

create policy profiles_self_write on profiles
  for update using (id = auth.uid()) with check (id = auth.uid());

-- Orgs are created through sugat.create_organization(), never by direct
-- insert — a bare insert could not also grant the creator their admin row.
create policy orgs_read on organizations
  for select using (sugat.is_org_member(id));

create policy orgs_admin_update on organizations
  for update using (sugat.is_org_admin(id)) with check (sugat.is_org_admin(id));

create policy memberships_read on memberships
  for select using (sugat.is_org_member(org_id) or profile_id = auth.uid());

create policy memberships_admin_write on memberships
  for update using (sugat.is_org_admin(org_id)) with check (sugat.is_org_admin(org_id));

create policy memberships_admin_delete on memberships
  for delete using (sugat.is_org_admin(org_id));

create policy invitations_admin_all on invitations
  for all using (sugat.is_org_admin(org_id)) with check (sugat.is_org_admin(org_id));

-- ---------------------------------------------------------------- social

create policy posts_read on posts
  for select using (sugat.is_org_member(org_id));

create policy posts_author_insert on posts
  for insert with check (sugat.is_org_member(org_id) and author_id = auth.uid());

create policy posts_author_update on posts
  for update using (sugat.is_org_member(org_id) and author_id = auth.uid())
...
create policy posts_moderator_update on posts
  for update using (sugat.is_org_moderator(org_id)) with check (sugat.is_org_moderator(org_id));

create policy posts_author_delete on posts
  for delete using (author_id = auth.uid() or sugat.is_org_moderator(org_id));
...
create policy reactions_own_insert on reactions
  for insert with check (sugat.is_org_member(org_id) and profile_id = auth.uid());
...
create policy follows_own_write on follows
  ...
  with check (sugat.is_org_member(org_id) and follower_id = auth.uid());
...
create policy events_committee_insert on events
  for insert with check (sugat.can_manage_events(org_id) and created_by = auth.uid());
...
create policy events_committee_update on events
  for update using (
    sugat.is_org_admin(org_id)
    or (sugat.can_manage_events(org_id) and created_by = auth.uid())
  )
  with check (
    sugat.is_org_admin(org_id)
    or (sugat.can_manage_events(org_id) and created_by = auth.uid())
...
create policy registrations_read on registrations
  for select using (profile_id = auth.uid() or sugat.can_manage_events(org_id) or sugat.is_org_admin(org_id));

create policy registrations_own_insert on registrations
  for insert with check (sugat.is_org_member(org_id) and profile_id = auth.uid());

create policy registrations_own_update on registrations
  for update using (profile_id = auth.uid() or sugat.is_org_admin(org_id))
  with check (profile_id = auth.uid() or sugat.is_org_admin(org_id));
...
create policy tickets_read on tickets
  for select using (
    profile_id = auth.uid()
    or sugat.is_org_admin(org_id)
    or sugat.is_event_checker(event_id)
  );
...
create policy event_checkers_admin_write on event_checkers
  for all using (sugat.is_org_admin(org_id) or sugat.can_manage_events(org_id))
  with check (sugat.is_org_admin(org_id) or sugat.can_manage_events(org_id));

-- ---------------------------------------------------------------- messaging

create policy threads_read on threads
  for select using (sugat.is_thread_member(id));

create policy threads_insert on threads
  for insert with check (sugat.is_org_member(org_id) and created_by = auth.uid());

create policy thread_members_read on thread_members
  for select using (sugat.is_thread_member(thread_id) or profile_id = auth.uid());

create policy thread_members_insert on thread_members
  for insert with check (sugat.is_thread_member(thread_id) or profile_id = auth.uid());

create policy thread_members_own_update on thread_members
  for update using (profile_id = auth.uid()) with check (profile_id = auth.uid());

create policy messages_read on messages
  for select using (sugat.is_thread_member(thread_id));

-- Broadcast threads are admin-to-org and one way; members read but never post.
create policy messages_insert on messages
  for insert with check (
    author_id = auth.uid()
    and sugat.is_thread_member(thread_id)
    and (
      sugat.is_org_admin(org_id)
...
create policy subscriptions_read on subscriptions
  for select using (profile_id = auth.uid() or sugat.is_org_admin(org_id));
...
create policy reports_insert on moderation_reports
  for insert with check (sugat.is_org_member(org_id) and reporter_id = auth.uid());

create policy reports_moderator_read on moderation_reports
  for select using (sugat.is_org_moderator(org_id) or reporter_id = auth.uid());
